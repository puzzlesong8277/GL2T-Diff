...
from datasets.pelvic import pelvic
from datasets.IXI import IXI
...
logging.basicConfig(filename='image_metrics_IXI_T2toPD.log', level=logging.INFO, format='%(asctime)s - %(message)s')
...
class Diffusion(object):
    ...
    def sg_train(self):
        ...
        if self.args.dataset=='IXI':
            # IXI for brain image translation
            dataset = IXI(self.config.data.train_dataroot, self.config.data.image_size, split='train')
            print('Start training your Fast-DDPM model on IXI dataset.')
        if self.args.dataset=='pelvic':
            # pelvic for brain image translation
            dataset = pelvic(self.config.data.train_dataroot, self.config.data.image_size, split='train')
            print('Start training your Fast-DDPM model on pelvic dataset.')

        if self.args.dataset=='BRATS2024':
            # pelvic for brain image translation
            dataset = BRATS(self.config.data.train_dataroot, self.config.data.image_size, split='train')
            print('Start training your Fast-DDPM model on BraTS2024 dataset in .',self.config.data.train_dataroot)
    ...
    def sg_ddpm_train(self):
        ...
        if self.args.dataset=='IXI':
            # IXI for brain image translation
            dataset = IXI(self.config.data.train_dataroot, self.config.data.image_size, split='train')
            print('Start training DDPM model on IXI dataset.')
        ...
        elif self.args.dataset=='pelvic':
            # BRATS for brain image translation
            dataset = pelvic(self.config.data.train_dataroot, self.config.data.image_size, split='train')
            print('Start training DDPM model on pelvic dataset.')
    ...
    def sg_sample_fid(self, model):
        ...
        if self.args.dataset == 'IXI':
            # IXI for brain image translation
            sample_dataset = IXI(self.config.data.sample_dataroot, self.config.data.image_size, split='calculate')
            print('Start training model on IXI dataset.')
        if self.args.dataset == 'pelvic':
            # pelvic for brain image translation
            sample_dataset = pelvic(self.config.data.sample_dataroot, self.config.data.image_size, split='calculate')
            print('Start training model on pelvic dataset.')
        elif self.args.dataset == 'BRATS':
            # BRATS for brain image translation
        ...
        with torch.no_grad():
            ...
            for batch_idx, sample in tqdm.tqdm(enumerate(sample_loader), desc="Generating image samples for FID evaluation."):
                ...
                for i in range(x.shape[0]):
                    ...
                    # 将每张图像的PSNR和SSIM存储到日志中
                    #logging.info('Image: {}, PSNR: {}, SSIM: {}'.format(case_name[i], psnr_temp, ssim_temp))
            ...
            # 在这里可以直接保存到日志文件，而不需要返回
            logging.info("Image metrics saved directly in the log.")
